...
const idQuery = 'SELECT empid,LoginPwd,empname FROM V_WHOIS_USER WHERE empid =';
...
module.exports = function(passport) {
  ...
  passport.use('local-signin', new LocalStrategy({
    ...
    passReqToCallback: true
      ...
      try{
        var user1=await User.findOne({userid:userid});
        if(user1 && await user1.validatePassword(password)){
          console.log(user1);
          return done(null,user1,req.flash('success','어서오세요'));
        }
        await sql.close()
        ...
        console.log(result1);
        if(!result1.recordsets){
          return done(null, false, req.flash('danger','사번이 존재하지 않습니다'))
        }
        var user=new User({
          'userid':userid,
